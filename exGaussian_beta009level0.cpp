...
//    return 0;
    ...
    for (int i = 0; i < ::NGROUPS; i++)
    {
        ...
        {
            ::MCMCCHAINS[i][j][0][0] = 1;
            ::MCMCCHAINS[i][j][1][0] = FsampleMuFromPrior     ();
            ::MCMCCHAINS[i][j][2][0] = FsampleSigmaFromPrior  ();
            ::MCMCCHAINS[i][j][3][0] = FsampleLambdaFromPrior ();
        }
    }
    ...
    cout << "MCMCCHAINS[0][0][0][0]=" << MCMCCHAINS[0][0][0][0];
    cout << "MCMCCHAINS[0][0][1][0]=" << MCMCCHAINS[1][0][1][0];
    cout << "MCMCCHAINS[0][0][2][0]=" << MCMCCHAINS[2][0][2][0];
    cout << "MCMCCHAINS[0][0][3][0]=" << MCMCCHAINS[3][0][3][0];
        ...
        for (int g109 = 0; g109 < ::NGROUPS; g109++)
        {
            ...
            if (prob01 < ::PMIGRATE)
            {
                ...
                for (int g = 0; g < number2migrate; g++)
                {
                    ...
                    {
                        group    = trunc(groupAndParticleVector[g]);
                        particle = 1000 * (groupAndParticleVector[g] - group);
                        ::HOLDINGPEN[group][particle][e177] = ::MCMCCHAINS[group][particle][e177][i88];
                    }
//                    for (int e187 = 1; e187 < ::NPARAMETERSLOTS; e187++)
                    for (int e187 = 0; e187 < ::NPARAMETERSLOTS; e187++)
                    {
                        int e187minusone;
                        hpgroup = group;
                        hpparticle = particle;
                        ...
                        {
                            e187minusone = e187 - 1;
                            ::HMIGRATION.parameter[mcmcgroup][mcmcparticle][e187minusone][i88] = :: HOLDINGPEN[hpgroup][hpparticle][e187];
                        }
                    }
        ...
        for (int g122 = 0; g122 < ::NGROUPS; g122++)
        {
            for (int p122 = 0; p122 < ::NPARTICLESPERGROUP; p122++)
            {
                ...
                if (prob02 < ::PMUTATE)
                {
                    ...
                    FwriteParticlePLL(g122,p122,i88,0,1);
                    ...
                    logMHprob = computeMHprob (g122, p122, i88, 0, tempLogQratio);
                ...
                else
                {
                    ...
                    if (i88 < ::BURNIN)
                    {
                        ...
                        FwriteParticlePLL(g122,p122,i88,0,3);
...
void FwriteParticlePLL (int group, int particle, int iteration, int mcmcchainsYN, int stage)
{
    ...
    mu     = mcmcchainsYN*::MCMCCHAINS[group][particle][1][iteration] + (1-mcmcchainsYN)*::HOLDINGPEN[group][particle][1];
    sigma  = mcmcchainsYN*::MCMCCHAINS[group][particle][2][iteration] + (1-mcmcchainsYN)*::HOLDINGPEN[group][particle][2];
    lambda = mcmcchainsYN*::MCMCCHAINS[group][particle][3][iteration] + (1-mcmcchainsYN)*::HOLDINGPEN[group][particle][3];
...
void Fmutation (int groupNumber, int particleNumber, int iterationID)
{
    ...
    for (int q205 = 1; q205 <= ::NPARAMETERS; q205++)
    {
        ...
        double tempvar207 = ::MCMCCHAINS[groupNumber][particleNumber][q205][iterationID] + Frandom_normal(0,.1);
        if (::LOLIMITS[0][q205] == 0)
        {
            ...
            else // upper limit but no lower one
            {
                ...
                {
                    tempvar207 = ::MCMCCHAINS[groupNumber][particleNumber][q205][iterationID] + Frandom_normal(0,.1);
                }
                ::HOLDINGPEN[groupNumber][particleNumber][q205] = tempvar207;
...
void fMakeMCMC (double MCMCCHAINS[::NGROUPS][::NPARTICLESPERGROUP][::NMCMCPARAMETERS][NITERATIONS])
{
    ...
    for (int g = 0; g < ::NGROUPS; g++)
    {
        for (int p = 0; p < ::NPARTICLESPERGROUP; p++)
        {
            ...
            output_file << "newInMCMC, MCMCMu, MCMCSigma, MCMCLambda, MCMCPLL, MigrateMu, MigrateSigma, MigrateLambda, MigratePLL, MutateMu, MutateSigma, MutateLambda, MutatePLL, CrossMu, CrossSigma, CrossLambda, CrossPLL  \r";
    ...
    ofstream output_file;
    output_file.open(filename.c_str());
    output_file << "group, particle, iteration, Mu, Sigma, Lambda, PLL  \r";
    for (int g = 0; g < ::NGROUPS; g++)
    {
        for (int p = 0; p < ::NPARTICLESPERGROUP; p++)
        {
...
void FupdateMCMC (int group, int particle, int iteration)
{
    ...
    //
    double logMHprob;
    logMHprob = computeMHprob (group, particle, iteration, 0, tempLogQratio);
    if (logMHprob > 0)
    { // keep it for sure
        ::MCMCCHAINS[group][particle][0][iteration] = 1;
        for (int w = 1; w <= ::NPARAMETERSANDONE; w++)
        {
